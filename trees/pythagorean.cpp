...
void pyth::triangle(Line bottom_line, std::vector<Line>& next_lines, std::vector<Line>& all_lines){
    sf::Vector2f middle = bottom_line.point_on_line_from_start(bottom_line.length / 2.f);
    sf::Vector2f middle_offset = middle - sf::Vector2f{0.f, bottom_line.length / 2.f};

    float angle_rad = std::atan2(bottom_line.end.y - bottom_line.start.y, bottom_line.end.x - bottom_line.start.x);
    float angle_deg = angle_rad * 360.f / (2 * PI);
    sf::Vector2f top = rotate_point_around_anchorpoint(middle_offset, middle, angle_deg);

    Line left(
        bottom_line.start,
        top,
        std::sqrt(2) * bottom_line.length / 2.f
    );
...
std::vector<Line> pyth::iterate(const std::vector<Line>& prev_lines, std::vector<Line>& all_lines){
    std::vector<Line> next_lines = {};
    for (Line line : prev_lines) {
        Line triangle_bottom = pyth::square(line, all_lines);
        pyth::triangle(triangle_bottom, next_lines, all_lines);
    }
    return next_lines;
}
...
std::vector<Line> pyth::simulate(const Config& config){
    ...
    for (int i = 0; i < config.iterations; i++) {
        start_lines = pyth::iterate(start_lines, all_lines);
    }
